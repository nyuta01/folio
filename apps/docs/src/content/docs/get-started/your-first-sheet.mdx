...
In a hurry? `folio init --name customers` scaffolds a runnable sheet
(starter `contract.yaml` / `records.jsonl` / `README.md` / one skill)
and the rest of this page is then about replacing the stubs with the
real customer schema. See [`folio init`](/folio/cli/init/) for the
full output.

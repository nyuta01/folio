...
SELECT-only ([ADR-0005](/reference/adrs/)). Anything else → `400 QueryError`.

### `GET /api/status?targets=country_code,industry_tag`

Returns the `materialization_status` shape (see [`folio status`](/cli/status/)).
...
`materialize.error`) frames on the [SSE event bus](/viewer/events/).
...
Server-Sent Events stream. See [Events](/viewer/events/) for the frame
format.
...
Neither is built-in. The Viewer is local-only by design ([Overview](/viewer/overview/)).

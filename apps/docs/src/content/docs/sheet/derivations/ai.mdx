...
[`AnthropicClientAdapter`](/folio/sdk/ai-client/) is the only module that imports
the `anthropic` SDK ([ADR-0009](/folio/reference/adrs/)), and a deterministic
...
([ADR-0009](/folio/reference/adrs/)). The `total_cost` on the
[`materialize` envelope](/folio/cli/materialize/) is the sum of all known costs in
the run.

## Running offline with the StubAIClient

For tests and the offline materialize smoke, inject a [`StubAIClient`](/folio/sdk/ai-client/):
...
- [AIClient SDK reference](/folio/sdk/ai-client/) — Protocol, adapter, stub.
- [Custom AI provider](/folio/sdk/ai-client/#writing-your-own-aiclient) —
  implement the Protocol against any model.
- [provenance.jsonl](/folio/sheet/provenance/) — what an `ai` provenance line

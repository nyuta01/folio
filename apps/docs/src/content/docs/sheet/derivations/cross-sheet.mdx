...
| `source_sheet` | yes | Relative path to the foreign sheet. Absolute paths are rejected, and the resolved path must stay inside the calling sheet's parent directory. |
...
The foreign directory must be a Folio sheet: Folio requires both
`contract.yaml` and `records.jsonl` before it reads values. This keeps
`cross_sheet` as a sibling-sheet join rather than a general local records
reader.

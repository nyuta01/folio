...
The venv stays outside the sheet for portability ([ADR-0008](/reference/adrs/)).
...
`output_schema` keys all show up on the [materialize envelope](/cli/materialize/)
...
hand-crafting a Python invocation. See the [`script` CLI verb](/cli/script/).
...
Reach for [`sql`](/sheet/derivations/sql/) when the transform is
expressible as a SELECT against `records`. Reach for [`http`](/sheet/derivations/http/)
when the source is over the network. Reach for [`ai`](/sheet/derivations/ai/)

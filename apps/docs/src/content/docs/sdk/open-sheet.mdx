...
operations documented in [Sheet API](/sdk/sheet-api/).

## Behaviour

- **Reads `contract.yaml` immediately.** The contract is validated against
  the [Folio spec](/sheet/contract/); failures raise `ContractError`.
...
| `ContractError: ...` | `contract.yaml` is missing, malformed, or fails [validation](/sheet/contract/). |

...
[Viewer](/viewer/overview/) — a local-only FastAPI + React app — against
the given sheet.
...
| Flag | Default | Notes |
|---|---|---|
| `--host` | `127.0.0.1` | Local-only by design ([§19.1](/viewer/overview/)). |
| `--port` | `3000` | |
...
- [Viewer overview](/viewer/overview/) — full architecture.
- [REST API](/viewer/rest-api/) — every endpoint.
- [Events (SSE)](/viewer/events/) — `/events` stream of materialize
  lifecycle frames.

...
`next_cursor`. Built for agent and tool consumption through the CLI or
the Viewer's `/api/records` endpoint.
...
The same SELECT-only rules from `folio query` apply to the filter —

...
A normative reference. For tutorial coverage, see [contract.yaml](/folio/sheet/contract/).
...
| `x-editable-by` | array of strings | – | Optional field ACL. When set, `fnmatch` patterns are matched on direct writes and materialize target writes. |
...
- [ADR-0003](/folio/reference/adrs/) — ODCS subset for contract.yaml.
- [ADR-0004](/folio/reference/adrs/) — JSONL records.
- [ADR-0007](/folio/reference/adrs/) — fnmatch for `x-editable-by`.

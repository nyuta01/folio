...
`query(sql, params)` verb against a DuckDB view named `records`. §10.4
...
constructs a typed `records` view from the contract, executes the user's
SQL, and returns rows as `list[dict]`.
...
- The operation count stays at the nine listed in §10.2, with no
  ...
  verbs.
- Users get the full DuckDB function library (window functions,
  aggregations, regex, JSON path) without Folio-specific glue.
...
parameterized filters return the expected rows. `tests/test_cli.py`
exercises the same rejection through the CLI, and `scripts/smoke-cli.sh`
asserts that `folio query <sheet> "DELETE FROM records"` exits non-zero.
